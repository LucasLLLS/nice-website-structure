...
function showEditBar(a,b,c,d){$("#editbar").slideUp(100);$("#editbar").slideDown(100);$(".fileDiv,.fullWidthFileDiv").removeClass("selected");$("div[data-imgid\x3d'"+c+"']").addClass("selected");$("#updates").css("visibility","hidden");$("#updates").slideUp(150);

$("#editbarDelete").attr("onclick","deleteImg('"+d+"', '"+c+"');");

$("#editbarUse").attr("onclick","useImage('"+a+"')");$("#editbarView").attr("onclick","showImage('"+a+"','"+b+"')");$("#editbarDownload").attr("href",a)}
...
function useImage(a){
    ...
    window.opener.CKEDITOR.tools.callFunction(b&&1<b.length?b[1]:null,a);
    window.close()
}

function uploadImg(){$("#uploadImgDiv").show();$("#background2").slideDown(250,"swing")}function pluginSettings(){$("#settingsDiv").show();$("#background3").slideDown(250,"swing")}
...
$(document).ready(function(){$("#dropzone").click(function(){setTimeout(function(){toggleDropzone("hide")},500)})});

function deleteImg(a,b){
    $.ajax({method:"GET",url:"imgdelete.php",data:{img:a}})
    .done(function(){var a=$("div[data-imgid\x3d'"+b+"']");

    "block"==Cookies.get("file_style")?a.addClass("deleteAnimationBlock"):a.slideUp(250,"swing");
